  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <motion.button
        ...
        className="w-48 group relative flex items-center gap-2 px-4 py-2.5 bg-background/80 hover:bg-background/90 
        rounded-lg transition-all duration-200 border border-border hover:border-border/80"
      >
        {/* hover state bg decorator */}
        <div className="absolute inset-0 bg-gradient-to-r from-primary/5 to-secondary/5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity" />

        <Palette className="w-4 h-4 text-muted-foreground group-hover:text-muted-foreground/80 transition-colors" />
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-full left-0 mt-2 w-full min-w-[240px] bg-background/95 
          ...
          >
            ...
            {THEMES.map((t, index) => (
              <motion.button
                ...
                className={`
                relative group w-full flex items-center gap-3 px-3 py-2.5 hover:bg-muted/50 transition-all duration-200
