...
const UserCohort = ({
  ...
  const isInstructor = orgUsers.instructors.some(
    (instructor) => instructor.id === user?.id
  );
  const isLearner = orgUsers.learners.some(
    (learner) => learner.id === user?.id
  );
  ...
  const handleGoToCourse = (course: Course) => {
    ...
    if (
      ...
      router.push(
        `/organizations/${orgId}/cohorts/${cohortId}/courses/${course.courseId}`,
        {
          scroll: false,
        }
      );
